...
| H10 Context Pipeline | 完成 | 2026-08-03 | Context Provider Pipeline（§16）：`src/harness/contracts/context.ts` + `src/harness/context/{pipeline,dedupe,budget-allocator,assemble,request}.ts` + `providers/*`（13 个，按层分组）—— ContextProvider（id/layer/priority/cacheable）→ ContextContribution（§16.2 + `group`/`priority` 扩展）→ 七步 Pipeline（收集→去重（cacheKey+sourceRefs 双通道）→新鲜度→排序→预算→裁剪→ContextSlice）；**验收达成：Loop 不再直接拼装 Context 来源**——round.ts 原 :133-213 拼装块替换为 `runContextPipeline` + `contextSliceToMessages`，`buildContextMessages` 已删除（参考实现内联进 freeze 测试）；**字节级行为冻结**（§3.5/§23）：13 个 provider 逐一包装现有构建函数（lang/stable-memory/kernel/context-map/skills/plan-state/research/staged/thinking/knowledge(round.ts 内联格式，非 KnowledgeBase.buildContext)/planning/mode/tail），stable 组 round≥1 经 frozenStablePrefix 逐字节透传（修复过 part() priority 硬编码 0 的排序 bug 与 frozen 双重头的组装配 bug）；**预算裁剪默认关闭**（现有代码无裁剪，开启需独立 PR + Golden 更新）；`tests/harness_context.test.ts` 23 项（顺序/预算/必需保留/去重/CacheKey/过期/映射表/逐 provider 字节）+ `harness_context_freeze.test.ts` 4 项（参考等价 JSON 深等/跨轮稳定/轮级回归）；全量 158 文件门禁绿色（agent_loop/l0 Golden/l7/context_epoch 零改动作冻结回归）。**技术债：** conversation-tail 为纯元数据（真 tail 预算 H11 Node 上下文）；`ContextLayer.node` 已入契约待 H11 使用；预算裁剪模式实现就绪未接线；mode 归 volatile 层（语义归 plan 的争论留 H11） |
| H11 Unified Node Runtime | 完成 | 2026-08-03 | Unified Node Runtime（§17 + GEP §5.4/5.5）：`src/harness/contracts/nodes.ts` + `src/harness/nodes/{context,events,run,function-node,tool-node,llm-agent-node,verification-node,human-node,index}.ts` —— HarnessNode（id/kind/execute→AsyncIterable<NodeEvent>/getResult→NodeResult，对齐 GEP §5.5 含 cancelled）；NodeEvent 独立旁路流（本地 sequence + nodeRunId，激活 H0 `EventEnvelope.nodeRunId` 字段，不占 run.eventSequence）；`runNode` 顺序执行器（**非 scheduler**，§23 约束：无 DAG/并行/动态 Workflow；single-use 强制）；**验收达成：单 Agent = 一个 LlmAgentNode**——复用 LegacyLoopAdapter + buildLoopOptions（AgentOptions 构造与旧 Loop 结构性一致）+ H4 BudgetGuard（run 级单一 ledger），三路对照测试（直测 agentLoop / AgentHarness / LlmAgentNode：最终文本、工具调用序列、outcome、usage 全等）；ToolNode 包装 H9 executeCapability（node 模式：budget 落 run ledger、默认 artifactTracker → write 类 patch artifact 落库、取消透传）；VerificationNode 薄包 H8 `ingestVerificationWithArtifact`（result → 绑定 artifact + evidence，不重演需 kernel 轮状态的 bindVerificationToLedger）；HumanNode 为 interrupt 桥（node.interrupt → respond 回调 → H7 `validateJsonSchema` 校验；**不写 run 级 interrupt 状态**，防 §23 双写）；`tests/harness_nodes.test.ts` 16 项（生命周期/取消前置/Node Context 注入/single-use/Budget 超限无泄漏/Node Artifact/Verification/HumanNode 合法与非法载荷/缺省 not_implemented）+ `harness_llm_agent_node.test.ts` 6 项（三路对照 + budget guard + 验收）；全量 160 文件门禁绿色。**技术债/发现：** LlmAgentNode 的 NodeResult.evidence=[]（H12 接证据链）；Node Context 为注入方案（kernel 侧独立构造 + node 裁剪 + modeContract 富化 + conversation-tail 真预算 defer H12）；VerificationNode 瘦身（coordinator 全量接线待 Workflow 节点携带 kernel 轮状态）；**H4 BudgetGuard 对真实 kernel 累计 usage 事件会双计 token**（provider usage 是累计快照，guard 用 +=；当前 harness_budget 测试用固定值 provider 未暴露——H12 修正）；HumanNode 不做 run 级状态（H7 机械留 AgentHarness） |
...
**状态：完成。** 全部门禁绿色（typecheck / 158 文件测试 / build / npm pack / diff --check）。

实现要点：

1. **契约 + 实现分层**（H9 同构）：`src/harness/contracts/context.ts` 纯类型（ContextLayer/LAYER_ORDER/ContextContribution/ContextProvider/ContextRequest/ContextSlice/ContextBudgetPolicy/ContextTrimInfo/ContextPipelineOptions），不 import loop.ts；实现进 `src/harness/context/*`。**对 §16.2 的两处扩展**：`ContextContribution.priority`（provider priority 复制到贡献上，pipeline 排序/组装配需要）与 `group`（同组贡献合并为一条消息——stable-prefix/volatile-round 两个组承载字节冻结拼装）。
2. **Pipeline 七步**（`pipeline.ts` + `dedupe.ts` + `budget-allocator.ts`）：收集（Promise.all 保注册序）→ 去重（cacheKey 主通道 + sourceRefs 副通道，胜者规则：层更稳 > priority 更小 > 注册序更前，dropped 记 reason）→ 新鲜度（过期 required 保留 + warning，非 required 丢弃；默认 maxAge=Infinity 不丢）→ 排序（layer+priority+providerId 三键）→ 预算分配（§16.5：stable 超限按 priority 降序裁非 required；plan required 永不裁；volatile 优先裁；maxTotalTokens 兜底；仍超 → `budget_overrun_required` warning）→ 裁剪 → ContextSlice（byProvider/dropped/budget/cachePrefixKeys/warnings，H11 NodeExecutionContext.context 输入）。
3. **13 个 Provider**（`providers/{stable,plan,volatile}.ts`，逐一包装现有构建函数，复用不重写）：lang-instruction/stable-memory/project-kernel/context-map/skills（stable 组）/plan-state/research（plan 层）/staged-context/thinking/knowledge/planning/mode-contract/conversation-tail（volatile 层）。**knowledge 包装 round.ts 的内联格式**（`## 已学知识` + `问题/方案`），不是 `KnowledgeBase.buildContext`（字节不同）；mode-contract 放 volatile/90（位置优先于语义，与现状 mode push 在 planning 后一致）。
4. **round.ts 接线（验收达成）**：原 :133-213 拼装块替换为 `createContextRequest(ctx, round)` → `runContextPipeline`（budget 不传 → 字节冻结路径）→ `contextSliceToMessages`；`buildContextMessages`/`ContextMessageInput` 已删除（参考实现内联进 freeze 测试）。epoch/kernel announce/gate 链/estimateRoundTokens 不动；`planStateText` 从 slice 同源取（消息与 epochRollover preamble 一致）。
5. **字节冻结（§3.5/§23）**：stable 组 round 0 由 5 个 part 拼装（`## Stable Prefix Context\n[CACHE_ANCHOR:v3]` 头，逐字符复刻），round≥1 仅 stable-memory 携带 frozenStablePrefix 逐字节透传（其余 part 返回空；组装配器按"唯一非空成员含 anchor"透传）。**过程中修复两个 bug**：stable part() 的 priority 硬编码 0 导致 5 个 stable provider 排序退化到 providerId 字母序（stable 组消息跑到 lang 前）；frozen 透传判断依赖 `sorted.length === 1` 但其他 3 个空 part 也在组里导致双重头（改为过滤空后判断）。
6. **测试**（`tests/harness_context.test.ts` 23 项 + `harness_context_freeze.test.ts` 4 项）：顺序（layer+priority 排序/组位置/空跳过）、预算（disabled 全保留/stable cap 降序裁/volatile 先于 plan 非 required/total cap 兜底/极值保 required + warning）、去重（cacheKey/sourceRefs 双通道）、CacheKey 稳定（同请求两次全等）、过期（required 保留 + warning）、映射表（13 个 provider 身份）、逐 provider 字节（plan-state === buildPlanStateContext 同输入、knowledge 内联格式、frozen 透传等）、freeze 参考等价（pipeline vs legacy 装配 JSON 深等）/跨轮稳定/轮级回归（agentLoop 消息结构：lang→stable→plan→mode→prompt）。存量 agent_loop/l0 Golden/l7/context_epoch **零改动全绿**作行为冻结证明。

**H11 入口：** Unified Node Runtime —— `ContextLayer.node` 已入契约；`ContextSlice` 可直接成为 `NodeExecutionContext.context` 输入；conversation-tail 转真 tail 预算；node 模式预算裁剪接线（Golden 更新随行）。

---

## H11 实施记录（2026-08-03）

**状态：完成。** 全部门禁绿色（typecheck / 160 文件测试 / build / npm pack / diff --check）。

实现要点：

1. **契约**（`src/harness/contracts/nodes.ts`，对齐 §17 + GEP §5.4/5.5）：`HarnessNode<I, O>`（§17.1 逐字）/`NodeKind`/`NodeResult<T>`（GEP §5.5 + cancelled：status/output/evidence/diagnostics/usage/retryable/error）/`NodeExecutionContext`（§17.3 逐字：runId/nodeRunId/runScope/capabilities/context: ContextSlice/budget/cancellation/artifacts/trace——**H9 的 CapabilityRegistry + H10 的 ContextSlice 字段直接就位**）。**NodeEvent 设计**：独立旁路流（node.status/output/text/tool.call/tool.result/usage/error/interrupt/artifact 九类），本地 sequence + nodeRunId（激活 H0 `EventEnvelope.nodeRunId` 字段，不改事件 schema），**不占 run.eventSequence**（run 流保持连续；trace 消费者不得假设跨 node 流单调——本记录注明与 sequence 连续 DoD 的关系）。
2. **顺序执行器**（`nodes/run.ts` `runNode`）：**非 scheduler**（§23 约束，头注释声明：无 DAG/edges/并行/动态 Workflow）；status 事件单一来源（running 开头 + 终态收尾）；**single-use 强制**（WeakSet：execute → getResult 恰一次，防 H12 scheduler 误用）；每个 yield 事件统一 best-effort 写 trace（`nodes/events.ts` emitter）。
3. **NodeExecutionContext 装配**（`nodes/context.ts`）：`createNodeExecutionContext({run, nodeRunId?, capabilities, context?})`——**budget = run.budget 单一 ledger**（node 模式不建第二 ledger，防 H4/H9 预算双计）；context 为**注入方案**（调用方预构建 slice；kernel 侧独立 ContextRequest 构造 + node 裁剪 + modeContract 富化 + conversation-tail 真预算 defer H12，涉及模型可见字节需 Golden 更新）。
4. **ToolNode**（包装 H9 `executeCapability` node 模式）：budget 落 run ledger（reserve/commit 与 H4 同语义）、abortSignal 透传取消、默认 artifactTracker（`createToolArtifactTracker`）→ **write 类 capability 成功后 patch artifact 自动落库（Node Artifact 验收）**；getResult 用 descriptor.retryable；catch 保留 HarnessError kind（如 capability_not_found）。**顺带修复 executor node 模式合成 bug**：metadata 曾取整个 output 对象（嵌套 patchTransactionId 丢失），改为取 output.metadata。
5. **LlmAgentNode（验收核心）**：`createLlmAgentNode({id, deps})`——**复用 LegacyLoopAdapter + buildLoopOptions**（AgentOptions 构造与旧 Loop 结构性一致，继承 plan_ready→plan artifact 等已接线行为）；`buildLoopOptions` 签名放宽为 `LoopRunContext`（Pick<AgentRun, runId|sessionId|scope>，纯类型改动零行为变化）；**H4 BudgetGuard 复用**（run 级 ledger，逐事件 observe，超限 → cancel + iterator.return 关闭资源）；事件翻译 text.emitted→node.text / tool.call→node.tool.call / model.usage→node.usage（**取最后 provider 值而非累加**——kernel 的 provider usage 是累计快照，累加会双计）/ plan_ready→node.interrupt；getResult 用 `mapDecisionToOutcome` 映射（completed/paused 有 text→succeeded；waiting→blocked interrupt_pending；取消→cancelled）。**验收测试**：三路对照（直测 agentLoop / AgentHarness / LlmAgentNode 同一假 provider）：最终文本、工具调用序列（name+input 深等）、outcome kind、usage 数值全等；"单 Agent = 一个 LlmAgentNode"（不经 AgentHarness 完成完整单 Agent 运行）。
6. **VerificationNode**（薄包 H8 `ingestVerificationWithArtifact`）：每 result → 绑定 artifact + evidence（`node.artifact` 事件）；不可分类 kind → warning 不失败；**明确边界**：不重演 `bindVerificationToLedger`（需 kernel VerificationContext），coordinator 全量接线等 Workflow 节点携带轮状态（H12 后）。
7. **HumanNode**（interrupt 桥）：node.interrupt → respond 回调（缺省 HarnessError internal "no responder"）→ H7 `validateJsonSchema` 校验（schema 按 kind：PLAN_APPROVAL_SCHEMA/CLARIFICATION_SCHEMA）；非法载荷 → failed；**不写 run 级 interrupt 状态**（run.interrupt/validateResume/markInterruptAnswered 留 AgentHarness，防 §23 双写）。
8. **测试**（`tests/harness_nodes.test.ts` 16 项 + `harness_llm_agent_node.test.ts` 6 项）：生命周期事件序、取消前置（handler 不执行）、Node Context（minimal slice 注入可见）、single-use 强制、context 装配（runId/scope/budget/cancellation/artifacts/trace）、ToolNode 成功/未知 capability/预算超限无 ledger 泄漏/patch artifact 落库/取消透传、VerificationNode 绑定与 unclassified warning、HumanNode 合法/非法/缺省 responder、三路对照（文本/工具序列/outcome/usage）、budget guard 取消、验收（不经 AgentHarness 完整单 Agent 运行）。

**H12 入口：** Eval Harness 2.0 —— Node 接口稳定后建立可信回归（Function/Run Replay、Provider/Tool Script、Trace Invariant、Rubric、基线比较）；同时清偿 H11 技术债：LlmAgentNode 证据链（NodeResult.evidence）、Node Context 富化（kernel 侧构造 + node 裁剪 + modeContract）、**H4 BudgetGuard 累计 usage 双计修正**、VerificationNode 全量接线。
