...
> [!IMPORTANT]
> **历史协调日志，不是当前运行状态或无人值守指令。** 本文保留 2026-08-06 至 2026-08-07 的窗口、领地、提交与事故记录，不能据此判断当前仍有这些进程/窗口，也不能据此启动、停止或接管任务。当前版本、源码、Linux 降级与发布状态以 [`../status.md`](../status.md) 和 [`../linux-foundation/current-status.md`](../linux-foundation/current-status.md) 为准。
