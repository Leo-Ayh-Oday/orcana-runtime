...
| Completion / Evidence | Done | `src/agent/completion-orchestrator.ts`, `src/agent/evidence-ledger.ts`, `src/agent/evidence-staleness.test.ts`, `src/agent/rollback-evidence.test.ts`, `tests/completion_orchestrator.test.ts` | Latest-result, write-generation freshness, fixed built-in verifier identity, unmanaged-write invalidation, authoritative commit-history identity, and **SS-Next-2B** rollback-aware invalidation are enforced; command-to-file coverage and finer-grained invalidation remain deferred |
