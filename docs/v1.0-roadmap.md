...
> [!IMPORTANT]
> **历史路线图快照，不是当前版本清单。** 本文保留 `v0.4.0` 规划基线和当时的里程碑编号。当前公开版本、源码候选、子系统状态与发布门禁以 [`status.md`](status.md) 为准；不要从本文的“当前基线”推断 npm、GitHub Release 或活动源码版本。
...
```
v0.4.0 — 当前基线（Phase 0-6 + ALK 减重 L0–L7 + Harness 2.0 H0–H8，全量门禁绿）
...
基于对 `main` 分支 README、ARCHITECTURE、`src/**`、`tests/**` 与关键官方资料的交叉审查，Orcana Runtime 已具备"强约束、强门控、单智能体工程循环"的鲜明风格，尤其在入口分诊、完成门控、运行时约束、DeepSeek 特性利用和中文工程化表达上，明显优于普通"会读会改会跑"的壳式 coding agent。
