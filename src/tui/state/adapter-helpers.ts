...
export function appendAssistantText(current: string, chunk: string): string {
  ...
  const maxLiveChars = Number(process.env.ORCANA_TUI_LIVE_CHARS ?? "12000")
...
export function summarizeUserPromptForTranscript(text: string): string {
  ...
  const maxInlineChars = Number(process.env.ORCANA_TUI_USER_DISPLAY_CHARS ?? "1600")
  const maxInlineLines = Number(process.env.ORCANA_TUI_USER_DISPLAY_LINES ?? "12")
